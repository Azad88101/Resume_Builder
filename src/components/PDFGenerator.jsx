...
// import ResumeDocument from "./ResumeDocument"; // Make sure to import your PDFDocument component
...
const PDFGenerator = ({ formData }) => {
  return (
    <div className="p-4 bg-[#1E1E1E] text-white rounded-lg border-2 text-center">
      <h2 className="text-xl font-semibold mb-4">Download Your Resume</h2>
      <PDFDownloadLink document={<ResumeDocument formData={formData} />} fileName={`${formData.name}_resume`}>
        {({ loading }) =>
          loading ? (
            <button className="px-4 py-2 bg-gray-400 text-white rounded">Generating PDF...</button>
          ) : (
